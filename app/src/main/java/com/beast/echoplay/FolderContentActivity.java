...
public class FolderContentActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        RecyclerView recyclerViewFolderContent = findViewById(R.id.recyclerViewMedia);
        recyclerViewFolderContent.setLayoutManager(new LinearLayoutManager(this));

        Intent intent = getIntent();
        String folderName = intent.getStringExtra("folderName");
        List<MediaItem> mediaItems = (List<MediaItem>) intent.getSerializableExtra("mediaItems");

        if (mediaItems != null) {
            MediaAdapter mediaAdapter = new MediaAdapter(this, mediaItems);
